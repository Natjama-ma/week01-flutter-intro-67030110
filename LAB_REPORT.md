    ...
    └─ Scaffold
        ...
        └─ Padding
           └─ Column
             ...
             │           │    ├── Text (คณะ: )
             │           │    └── Expanded
             │           │        └── Text (ครุศาสตร์อุตสาหกรรมและเทคโนโลยี)
             │           ├─ Divider
             │           ├─ Row (_buildInfoRow: วิชาที่ชอบ)
             │           ├─ Divider
             │           ├─ Row (_buildInfoRow: เป้าหมาย)
             │           ├─ Divider
             │           ├─ Row (_buildInfoRow: อีเมล)
             │           ├─ Divider
...
| ใช้เมื่อไหร่ | ปรับแต่ง UI, แก้ไขข้อความ/สี |แก้ไขโครงสร้างข้อมูล, แก้ไขในฟังก์ชัน(main/init)  |

### 3.4 ผลการทดลอง Prompt Engineering

**Prompt แบบ Simple:**
```
ช่วยเขียนโค้ดแอป Flutter เป็นการ์ดแสดงตัวจับเวลา Pomodoro ให้หน่อย

```

  <img width="617" height="611" alt="image" src="https://github.com/user-attachments/assets/0cf97f14-4409-4aed-bc20-8804ced7e259" />
...
```
ช่วยสร้าง Flutter Widget ชื่อ PomodoroHorizontalCard ด้วย StatefulWidget สำหรับตัวจับเวลา Pomodoro
โดยออกแบบ Layout ให้เป็นแนวนอน (Wider Card) เน้น UI ที่ดู Modern และพรีเมียม มีรายละเอียดดังนี้:
1. พื้นหลังการ์ด: ขอบมนมาก (borderRadius 30) ใส่เงาและไล่สีพื้นหลัง(Gradient)จากสีส้มเข้มไปเหลือง
2. แบ่ง Layout เป็น 2 ฝั่ง (ซ้าย-ขวา) ด้วย Row:
  - ฝั่งซ้าย (ส่วนจับเวลา): ใช้ Stack สร้างวงแหวน CircularProgressIndicator(สีขาว เส้นหนา 10) ที่ผูกค่าความคืบหน้าตามเวลาจริง
ซ้อนกับตัวเลขเวลา (เช่น '25:00') สีขาวตัวหนาขนาด 48 ไว้ตรงกลาง จัดให้อยู่กึ่งกลางสวยงาม
  - ฝั่งขวา (ส่วนจัดการ Session และโหมด): ใช้ Column จัดเรียงองค์ประกอบดังนี้:
     - ส่วนหัว (Mode Selector): สร้าง Row ที่มีปุ่ม 3 ปุ่มสำหรับเลือกโหมดคือ 'Focus' (25 นาที), 'Short Break' (5 นาที), และ
      'Long Break' (15 นาที) (ปุ่มที่เลือกให้มีพื้นหลังสีขาวโปร่งแสง ส่วนปุ่มอื่นโปร่งใส โดยบังคับให้กดติดด้วย HitTestBehavior.opaque)
     - ส่วนสถานะ (Session Tracker): ทำจุดวงกลมเล็กๆ 4 จุด (สีขาวทึบ 1 จุด โปร่งแสง 3 จุด) เพื่อแสดงรอบการทำงาน
     - ปุ่ม Action: ปุ่ม 'START / PAUSE' แบบ ElevatedButton ทรงแคปซูล พื้นหลังสีขาว ตัวหนังสือสีส้ม จัดปุ่มให้ขยายเต็มความกว้างของฝั่งขวา
       เมื่อกดให้สลับการทำงานของ Timer และเปลี่ยนข้อความบนปุ่มได้

```

  <img width="725" height="438" alt="image" src="https://github.com/user-attachments/assets/8dcaac4a-6d81-4e57-a9e6-0954c6297e04" />
...
```
1. ด้าน UI
Simple Prompt: หน้าจอดูเรียบๆ มีการออกแบบแบบพื้นฐาน มีเพียงวงกลมแสดงเวลาและปุ่มควบคุม 
Detailed Prompt: หน้าจอมีความ Modern มากขึ้น ใช้พื้นหลังแบบ Gradient มีองค์ประกอบ สีสันและปุ่มต่าง ๆ เป็นสัดส่วน ทำให้ UI ดูน่าใช้งานกว่าขึ้น
2. ด้านความครบถ้วนขององค์ประกอบ
Simple Prompt: แสดงเฉพาะตัวจับเวลาและปุ่มควบคุมพื้นฐานตามรูปแบบทั่วไป ยังไม่มีฟังก์ชันหรือข้อมูลเพิ่มเติม เช่น โหมดการทำงานหรือ Session
Detailed Prompt: มีองค์ประกอบครบตามที่กำหนดไว้ใน Prompt ได้แก่ โหมด Focus, Short Break, Long Break, การแสดง Sessions และปุ่ม Start
ที่ออกแบบตามที่ต้องการ อาจจะต้องปรับแก้ตามความต้องการเพิ่มเติมแต่จะลดเวลาในการแก้ไขโค้ดภายหลัง
...
<img width="1918" height="836" alt="image" src="https://github.com/user-attachments/assets/f0b34433-5b46-467d-b35d-6f565659b90b" />

<img width="1918" height="916" alt="image" src="https://github.com/user-attachments/assets/8fb1e2f5-e3c9-4ef8-ac6a-4d2b1ec3aa23" />
...
```
คำตอบ:
Flutter ใช้ Rendering Engine ของตัวเอง (Impeller หรือ Skia) ในการวาด UI โดยตรง ทำให้แสดงผลได้รวดเร็วและมีหน้าตาเหมือนกันทุกแพลตฟอร์ม
ส่วน React Native ใช้ Native Components ของระบบปฏิบัติการในการแสดงผล ซึ่งสื่อสารผ่าน Bridge เพื่อแปลงโค้ด
...
```
คำตอบ:
StatelessWidget: เป็น Widget ที่ข้อมูลจะถูกล็อกไว้ตายตัวตั้งแต่แรก เปลี่ยนแปลงเองไม่ได้ เหมาะกับส่วนที่ใช้แสดงผลเฉยๆ เช่น การ์ดแสดงข้อมูลโปรไฟล์ส่วนตัว
ข้อความ รูปภาพ หรือไอคอน
StatefulWidget: เป็น Widget ที่สามารถเปลี่ยนแปลงได้ระหว่างการใช้งาน เหมาะสำหรับหน้าจอที่มีการโต้ตอบ เช่น หน้าฟอร์มกรอกข้อมูล หรือตัวนับเวลา(Timer)
...
```
คำตอบ: เหตุผลที่ไม่ควร commit API Key เพราะ API Key เป็นข้อมูลลับ ถ้ามีผู้ไม่หวังดีนำ API Key ไปใช้งานแทนได้ ซึ่งอาจทำให้เกิดค่าใช้จ่าย ถูกขโมยข้อมูล
หรือถูกนำไปใช้ในทางที่ผิด

วิธีการจัดการ คือ แยกไปเก็บไว้้ในไฟล์ .env แล้วเอาชื่อไฟล์ไปใส่ใน .gitignore เพื่อไม่ให้โดนดึงขึ้นเซิร์ฟ และใช้ตัว Environment variables
ในการดึงมาใช้งานตอนรัน
...
```
คำตอบ: Hot Reload ทำงานโดยอัปเดตโค้ดที่แก้ไข เข้าไปใน Dart VM ที่กำลังรันอยู่บนเครื่องจำลอง ทำให้หน้าตาแอปอัปเดตทันทีในเสี้ยววินาทีโดยที่ค่าข้อมูลต่างๆ
ที่พิมพ์ค้างไว้ยังอยู่ครบ ข้อจำกัดคือมันจะใช้งานไม่ได้ถ้าเราไปแก้ไขโครงสร้างหลักของแอป เช่น การแก้ไขโค้ดใน main() การเพิ่ม Dependency หรือการเปลี่ยนแปลง
ที่กระทบการเริ่มต้นแอป อาจต้องใช้ Hot Restart หรือรันแอปใหม่
...
```
คำตอบ: 
1. ช่วยจัดระเบียบโค้ดที่ยาวเกินไป (Code Refactoring): เวลาเขียนแอปไปเรื่อยๆ แล้วโค้ดเริ่มยาวและซ้อนกันมั่วไปหมด เราสามารถโยนโค้ดนั้นให้ AI ช่วยจัดระเบียบ
ตัดแบ่งเป็นชิ้นย่อยๆ เพื่อให้โค้ดเราดูสะอาดและกลับมาแก้ทีหลังได้ง่ายขึ้น
2.  ช่วยในการวิเคราะห์ขอบเขตการทำงานของแอป (QA): ใช้ AI เป็นผู้ช่วยในการวิเคราะห์ขอบเขตการทำงานของแอป แล้วให้มันช่วยเจเนอเรต Test Case สำหรับ
ทดสอบระบบแบบครอบคลุม (เช่น เขียนเคสทดสอบ Input Validation ของแอปคำนวณ BMI) เพื่อลดภาระและประหยัดเวลา
3. ช่วยออกไอเดียและตกแต่งหน้าจอ (UI Styling): เวลาที่เราคิดไม่ออกว่าจะจัดหน้าจอยังไงให้สวย หรือไม่รู้จะจับคู่สีไหนดี เราสามารถพิมพ์บอก AI ได้เลยว่าอยากได้
แอปสไตล์ไหน แล้วให้มันช่วยแนะนำโค้ดสี หรือวิธีตั้งค่า Widget ให้ดูดีขึ้น
...
- [✓] `flutter doctor` ไม่มี `[✗]` (มี `[!] Android Studio` ได้ — ปกติสำหรับ VS Code Workflow)
    หมายเหตุ: มีสถานะ [X] ที่ Visual Studio เนื่องจากไม่ได้ติดตั้งชุดเครื่องมือสำหรับการพัฒนา Windows Desktop App
แต่ชุดเครื่องมือหลักสำหรับพัฒนาบน Android และ Web (Chrome) ติดตั้งสมบูรณ์และพร้อมใช้งานแล้ว
